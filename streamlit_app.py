...
# main page content
def main_page():
    _title = st.title("VSCO Downloader")

    general_description = """
    VSCO Downloader is a simple script to extract raw image and video paths from [VSCO](https://www.vsco.co/) posts.
    """
    _description = st.markdown(general_description)

    _header_1 = st.subheader("Extract Post Media", divider="rainbow")
    ...
    if download:
        if url is None or url.strip() == "":
            _url_err = st.error("URL of the post has to be provided!")
        else:
            media_urls = get_links(url, True)
            _header_2 = st.subheader("Extracted Post Media", divider="rainbow")
            _display_media_urls = st.markdown(
                "\n".join(
                    [
                        f"- Found media url: [{media_url}]({media_url})"
...
def main():
    ...
    st.set_page_config(
        page_title="VSCO Downloader",
        page_icon=":camera:",
        layout="wide",
        initial_sidebar_state="expanded",
        menu_items={
            "Get Help": "https://github.com/michabirklbauer/vsco_downloader/discussions",
            "Report a bug": "https://github.com/michabirklbauer/vsco_downloader/issues",
            "About": about_str,
        },
    )

    _title = st.sidebar.title("VSCO Downloader")

    _div_1 = st.sidebar.subheader("", divider="rainbow")

    _logo = st.sidebar.image(
        ".streamlit/ico/download-icon.png",
    ...
    _div_2 = st.sidebar.subheader("", divider="rainbow")
    ...
    _info = st.sidebar.markdown(info_str)
